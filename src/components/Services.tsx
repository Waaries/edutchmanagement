...
const Services = () => {
  ...
  return <section id="diensten" className="section-padding angled-bg" ref={sectionRef}>
      <div className="container mx-auto container-padding reveal">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {services.map((service, index) => <div key={index} className={`relative overflow-hidden transition-all duration-300 border-2 ${service.mostPopular ? 'border-primary shadow-xl shadow-primary/10' : 'border-slate-200'} hover-lift rounded-3xl bg-white flex flex-col`}>
              {service.mostPopular && <div className="absolute top-0 right-0 gradient-primary text-white px-4 py-1 text-sm font-medium rounded-bl-2xl rounded-tr-3xl">
              ...
              <div className="p-8 flex-grow flex flex-col">
                <div className="flex-grow">
                  ...
                  <ul className="space-y-3 mb-8">
                    {service.features.map((feature: string, idx: number) => <li key={idx} className="flex items-start">
                ...
                <Button className={`w-full mt-auto ${service.mostPopular ? '' : 'variant-outline'}`} variant={service.mostPopular ? "default" : "outline"}>
