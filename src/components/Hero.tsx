          ...
          <div className="lg:col-span-3 space-y-8 mx-auto text-center lg:text-left">
            <div className={`transition-all duration-1000 delay-300 ${isVisible ? 'opacity-100' : 'opacity-0 translate-y-10'}`}>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4">
                <Button className="px-8 py-6 group" onClick={() => scrollToSection('diensten')}>
                  <span>{translate("hero.discoverBtn")}</span>
                  <ArrowRight className="ml-2 h-4 w-4 transition-transform group-hover:translate-x-1" />
                </Button>
                <Button variant="outline" className="px-8 py-6" onClick={() => scrollToSection('contact')}>
            ...
            }].map((item, index) => <div key={index} className={`flex flex-col items-center lg:items-start gap-4 transition-all duration-700 delay-${(index + 1) * 200} hover:bg-primary/5 p-4 rounded-2xl`}>
          ...
          <div className="lg:col-span-2 flex justify-center lg:justify-end transition-all duration-1000 delay-500">
            <div className={`stacked-card ${isVisible ? 'opacity-100' : 'opacity-0 translate-x-10'}`}>
              <div className="glass-card rounded-3xl overflow-hidden relative z-10 card-shadow">
                <img src="https://images.unsplash.com/photo-1497366754035-f200968a6e72?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&q=80" alt="Modern office space" className="w-full h-[500px] object-cover object-center" />
