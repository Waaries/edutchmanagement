-- FINAL SECURITY HARDENING - Token Access & Profile Protection (Fixed)
...
-- 2. Add token complexity requirements (drop constraint first if exists)
DO $$ 
BEGIN
...
ADD CONSTRAINT token_length_check 
...
-- Update existing profiles RLS policy to be more restrictive
DROP POLICY IF EXISTS "Authenticated users manage own profile" ON public.profiles;
