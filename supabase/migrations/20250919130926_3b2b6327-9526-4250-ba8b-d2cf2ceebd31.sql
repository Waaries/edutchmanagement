...
-- First, ensure user_id column allows NULL temporarily for existing data
ALTER TABLE filled_contracts ALTER COLUMN user_id DROP NOT NULL;

-- Update any existing contracts with NULL user_id to a safe default
...
-- Drop ALL existing policies on filled_contracts
DO $$ 
BEGIN
...
-- Create new strict policies with unique names
CREATE POLICY "contracts_user_insert_only" ON filled_contracts FOR INSERT
WITH CHECK (user_id = auth.uid() AND user_id IS NOT NULL);

CREATE POLICY "contracts_user_select_own" ON filled_contracts FOR SELECT
USING (user_id = auth.uid() OR is_admin());

CREATE POLICY "contracts_user_update_own" ON filled_contracts FOR UPDATE
USING (user_id = auth.uid() OR is_admin())
WITH CHECK (user_id = auth.uid() OR is_admin());

CREATE POLICY "contracts_user_delete_own" ON filled_contracts FOR DELETE
USING (user_id = auth.uid() OR is_admin());
...
DO $$ 
BEGIN
    DROP POLICY IF EXISTS "Admin only rate limit access" ON rate_limit_tracking;
EXCEPTION 
    WHEN OTHERS THEN NULL;
END $$;

CREATE POLICY "rate_limit_admin_access" ON rate_limit_tracking FOR ALL
...
BEGIN
    ...
    DROP POLICY IF EXISTS "Authenticated users manage own profile" ON profiles;
EXCEPTION 
    WHEN OTHERS THEN NULL;
END $$;

-- Create strengthened profile policy with unique name
CREATE POLICY "profiles_auth_user_access_only" ON profiles FOR ALL
USING (
...
DO $$ 
BEGIN
    DROP POLICY IF EXISTS "Admin only profile access logs" ON profile_access_log;
EXCEPTION 
    WHEN OTHERS THEN NULL;
END $$;

CREATE POLICY "profile_logs_admin_only" ON profile_access_log FOR ALL
USING (is_admin()) WITH CHECK (is_admin());

-- Profile access logging and validation triggers
...
-- Clean up function for rate limit entries
